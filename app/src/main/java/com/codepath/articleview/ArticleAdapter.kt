...
class ArticleAdapter(private val context: Context, private val articles: List<Article>) :
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView),
        ...
        fun bind(article: Article) {
            ...
            abstractTextView.text = article.snippet
            Glide.with(context).load(article.smallImageUrl).skipMemoryCache(true).into(mediaImageView)
        }

        override fun onClick(v: View?) {
            // Get selected article
            val article = articles[bindingAdapterPosition]
